...
| **NFR-USE-01** | หน้าจอส่วนใหญ่ใช้โทนสีขาว-ฟ้าสะอาดตา จัดหมวดหมู่สินค้าตามประเภทอะไหล่ (อ้างอิง ihavecpu.com) *(หน้า Dashboard เปลี่ยนเป็นธีมมืดตาม CR-010 — ขอบเขตจำกัดเฉพาะหน้านั้น)* และผู้ใช้ใหม่ต้องทำ task ค้นหา/เช็คประกันสำเร็จ **≥90% ภายใน 60 วินาที** โดยไม่ต้องมีคนสอน | ความใช้งานง่าย | Should | Task-based user test |
...
| FR-014 *(CR-008, ธีมมืด+กราฟตาม CR-010)* | ทีมขอเพิ่ม 2026-08-24 | — (cross-cutting) | ใช้ endpoint เดิมที่มีอยู่แล้ว (`/api/stock`, `/api/products`, `/api/purchase-requests`, `/api/branches`) ไม่มี endpoint ใหม่ · กราฟ (`recharts`) แสดง breakdown จริงตามหมวดหมู่/สถานะ PR | ไม่มี backend test แยก (ไม่มี logic ใหม่ฝั่ง server) | ✅ Implemented — verify ผ่าน browser จริงทั้ง Admin (`admin/Dashboard.jsx`) และ Branch (`branch/Dashboard.jsx`) เห็นตัวเลข KPI + กราฟตรงกับข้อมูลจริงใน DB, responsive ตรวจแล้วบน mobile viewport |
...
| NFR-USE-01 | P4, P10, CR-010 | — (cross-cutting) | หน้าเว็บครบ 3 persona (Public/Branch/Admin) โทนขาว-ฟ้าตาม Tailwind brand colors — **ยกเว้นหน้า Dashboard (Admin+Branch) เป็นธีมมืดตาม CR-010** | ทดสอบด้วยมือผ่าน browser จริง (login→ขาย→PR→approve→audit log ครบ loop) ยังไม่มี usability test แบบมีผู้ใช้จริงมาวัด task success rate | 🟡 ใช้งานได้ครบ verify ด้วยตาเปล่าแล้ว **แต่ยังไม่มี formal usability test ตาม spec เดิม — ต้องการผู้ใช้จริงทดลอง AI ทำแทนไม่ได้ ทีมต้องทำเองก่อนส่งงาน** และหน้าตาระบบไม่สอดคล้องกันหลัง CR-010 (nav โทนอ่อน, dashboard โทนมืด) |

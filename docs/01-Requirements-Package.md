...
| FR-014 *(CR-008, ธีมสว่างสไตล์ Render+กราฟตาม CR-010)* | ทีมขอเพิ่ม 2026-08-24 | — (cross-cutting) | ใช้ endpoint เดิมที่มีอยู่แล้ว (`/api/stock`, `/api/products`, `/api/purchase-requests`, `/api/branches`) ไม่มี endpoint ใหม่ · กราฟแท่งแนวนอน (`recharts`) แสดง breakdown จริงตามหมวดหมู่/สถานะ PR สีผ่าน CVD-safety validator | ไม่มี backend test แยก (ไม่มี logic ใหม่ฝั่ง server) | ✅ Implemented — verify ผ่าน browser จริงทั้ง Admin (`admin/Dashboard.jsx`) และ Branch (`branch/Dashboard.jsx`) เห็นตัวเลข KPI + กราฟตรงกับข้อมูลจริงใน DB, responsive ตรวจแล้วบน mobile viewport |

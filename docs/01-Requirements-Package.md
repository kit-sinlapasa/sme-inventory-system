...
### Constraints
- ต้องพัฒนาและส่งงานภายในกรอบเวลาของวิชา NC221 (อ้างอิงเอกสารรายวิชา NC221 ซึ่งอยู่นอก repo นี้ สัปดาห์ 3–11)
...
| FR-012 | โน้ตหน้า 2 | US-08 | ER: `BRANCH_SKU.reorder_point` + `low_stock_alert_sent_at` · API: `GET/PUT /api/branch-sku/{branch_id}/{sku_id}` · `services/stock_alerts.py`, `services/notifications.py` (CR-006) | `test_branch_sku.py` (4 tests) + `test_stock_alerts.py` (3 tests: debounce ยิงครั้งเดียว, reset หลัง restock, ไม่ยิงถ้าไม่ตั้ง reorder point) | ✅ Implemented + Tested — ส่งอีเมลจริงยังไม่ verify (ไม่มี SMTP credential ให้ AI ทดสอบ ดู CR-006) แต่ debounce logic verify ครบผ่าน monkeypatch · **ผลตรวจจริงบน production (2026-08-25):** ตั้งค่า SMTP ครบทั้ง 6 ตัวแล้ว แต่ `GET /api/admin/smtp-check` (เพิ่มใน CR-016) รายงาน `connection: blocked` — `OSError: [Errno 101] Network is unreachable` ตอนต่อ `smtp.gmail.com:587` · **Render free tier บล็อก outbound SMTP** โค้ดตายตั้งแต่ขั้น connect ยังไม่ถึงขั้น login ด้วยซ้ำ จึงยืนยันได้ว่าไม่ใช่ปัญหาของโค้ดหรือ credential · **ตรรกะ "ควรแจ้งเมื่อไหร่" มี test ครอบคลุมและผ่าน** ส่วนการส่งออกจริงพิสูจน์ได้เฉพาะนอก Render |
...
| NFR-USE-01 | P4, P10, CR-010 | — (cross-cutting) | หน้าเว็บครบ 3 persona (Public/Branch/Admin) โทนขาว-ฟ้าตาม Tailwind brand colors — **ยกเว้นหน้า Dashboard (Admin+Branch) เป็นธีมสว่างสไตล์ dashboard.render.com ตาม CR-010 (ม่วง/ขาวแทนขาว-ฟ้า)** | ทดสอบด้วยมือผ่าน browser จริง (login→ขาย→PR→approve→audit log ครบ loop) · **[06-Usability-Test-NFR-USE-01.md](06-Usability-Test-NFR-USE-01.md)** — เดิน task จริงบน production วัดจำนวนขั้นตอน + heuristic evaluation (Nielsen 10) + สคริปต์ทดสอบพร้อมรันกับผู้ใช้จริง | ✅ **Implemented + Tested with real users (2026-08-25)** — ทดสอบกับ**ผู้ที่ไม่ได้พัฒนาระบบ 7 คน** (2 รอบ) — เป็นคนนอกกลุ่มโครงงาน 2 คน และสมาชิกกลุ่มที่รับผิดชอบงานส่วนอื่น 5 คน ทุกคนเพิ่งเห็นระบบครั้งแรกวันทดสอบ **สำเร็จ 7/7 ภายใน 60 วินาที** เฉลี่ย 21.7 วิ ช้าสุด 30 วิ (ครึ่งเดียวของงบ) · **ตัดผู้พัฒนาออกจากกลุ่มตัวอย่าง** หลังพบว่าคนที่เร็วที่สุด (10 วิ) คือคนสร้างระบบเอง — เก็บไว้เป็น baseline แทน: ผู้ใช้จริงช้ากว่าผู้พัฒนา 1.7-3.0 เท่า · แต่ละคนได้ S/N คนละหมวดสินค้า ไม่ให้ดูกัน ผู้ดำเนินการอ่านบทพูดตามสคริปต์ ไม่ชี้ทาง · **ส่วนที่วัดด้วยเครื่องมือผ่าน 10/10 บน production** (ช่องกรอกพร้อมพิมพ์ใน 0.6–1.0 วิ เมื่อเครื่องตื่น (~4.2 วิ ถ้า Render เพิ่งตื่น) · ทำ task ด้วยคีย์บอร์ดล้วนได้ · มีป้ายกำกับสำหรับโปรแกรมอ่านหน้าจอ · ใช้บนมือถือ 375px ได้) · **ข้อจำกัดที่ระบุไว้ตรง ๆ:** n=7 ยังไม่พอยืนยัน ≥90% เชิงสถิติ (rule of three ให้ขอบบนของอัตราล้มเหลว 43% — เคยแย่ลงเป็น 75% ตอนตัดผู้พัฒนาออก แล้วดีขึ้นเมื่อทดสอบเพิ่มตามที่ผู้รีวิว PR ติง) ต้องราว 30 คนจึงยืนยันได้ ต้องราว 30 คนจึงจะยืนยันได้ — จึงรายงานเป็น "7/7 คน" ไม่แปลงเป็นเปอร์เซ็นต์ [ดู docs/06](06-Usability-Test-NFR-USE-01.md) |
| NFR-A11Y-01 *(CR-015)* | เกณฑ์หมวด Accessibility ที่โจทย์กำหนด | — (cross-cutting) | UI: `pages/public/WarrantyCheck.jsx` — `aria-label` บนช่องกรอก, ปุ่ม `min-h-11` (44px) | `docs/screenshots/usability-checks.js` — วัดอัตโนมัติบน production 10 ข้อ | ✅ Implemented + Tested — **10/10 ผ่านบน production** (ช่องกรอกพร้อมพิมพ์ 0.6–1.0 วิ (เครื่องตื่น) · Tab แรกโฟกัสช่องกรอก · Enter ส่งฟอร์มได้ · มี `<h1>`/ปุ่มมีข้อความ/`lang=th` · 375px ไม่มีเลื่อนแนวนอน · ปุ่มสูง 44px) · **พบและแก้ 2 จุดจากการวัดนี้** — เดิมช่องกรอกมีแค่ `placeholder` (ขัด WCAG 3.3.2) และปุ่มสูง 38px |
...
- **ข้อมูลตัวอย่างไม่เท่ากันทุกสาขา** (ทีมขอให้สมจริง) — ใส่ `sku_coverage` (สัดส่วนรุ่นที่สาขาสต็อก) และ `size` (ตัวคูณจำนวนรับเข้า) ต่อสาขา ผลจริง ณ ตอนทำ CR-012: สำนักงานใหญ่ 60 รุ่น/191 ชิ้น → สาขาสยาม 55/112 → สาขารัชดา 40/50 → สาขารังสิต 28/21 **(ตัวเลขชุดนี้เป็นบันทึก ณ เวลานั้น — CR-013 เขียน seed ใหม่ทั้งหมด จำนวนจึงต่างไปแล้ว สิ่งที่ยังคงเดิมคือลำดับขนาดสาขาซึ่งเป็นประเด็นของ CR นี้)** · คำขอสั่งซื้อกระจายหลายสาขาแล้ว โดยสาขาเล็กขอบ่อยกว่าเพราะสต็อกน้อย (สำนักงานใหญ่เป็นคลังกลาง ไม่ต้องขอจากตัวเอง)
...
### CR-016: เพิ่ม endpoint วินิจฉัยการตั้งค่าอีเมล (`GET /api/admin/smtp-check`)
- **เหตุผล:** ตั้งค่า SMTP บน production ครบแล้วแต่อีเมลไม่ออก และ**แยกสาเหตุไม่ได้**
  เพราะทั้งสามกรณี (ตั้งค่าไม่ครบ / รหัสผ่านผิด / hosting บล็อกพอร์ต) มีอาการเดียวกันคือ "ไม่มีเมล"
  ยิงทดสอบไป 4 ครั้งก็ยังบอกไม่ได้ว่าติดตรงไหน
- **ขอบเขต:** ต่อ SMTP จริงแต่**ไม่ส่งเมลจริง** (connect + starttls + login แล้วตัดสาย)
  คืนสถานะ `not_configured` / `auth_failed` / `blocked` / `ok`
- **ความปลอดภัย:** endpoint ที่รายงานสถานะ credential เสี่ยงกลายเป็นช่องอ่าน credential
  ออกจากระบบ จึงคุม 3 ชั้นและมี test คุมทุกชั้น — Admin เท่านั้น · คืนแค่ boolean ไม่คืนค่าจริง ·
  ตัดค่าลับออกจากข้อความ error อีกชั้นเผื่อไลบรารีภายนอกใส่กลับมาเอง
- **ผลที่ได้:** `connection: blocked` — `[Errno 101] Network is unreachable`
  **Render free tier บล็อก outbound SMTP** ปิดคดีที่ค้างมาหลายรอบ
- **ตัดสินใจ:** Approved (2026-08-25) — เก็บ endpoint ไว้ไม่ถอดออก เพราะเป็น health check
  ที่มีประโยชน์จริงสำหรับคนที่มาตั้งค่าต่อ และเป็นหลักฐานประกอบ K-03

...
| FR-014 *(CR-008, ธีมสว่างสไตล์ Render+กราฟตาม CR-010)* | ทีมขอเพิ่ม 2026-08-24 | — (cross-cutting) | ใช้ endpoint เดิมที่มีอยู่แล้ว (`/api/stock`, `/api/products`, `/api/purchase-requests`, `/api/branches`) ไม่มี endpoint ใหม่ · กราฟแท่งแนวนอน (`recharts`) แสดง breakdown จริงตามหมวดหมู่/สถานะ PR สีผ่าน CVD-safety validator · ตารางสต็อกเพิ่ม client-side search (กรองตามสาขา/หมวดหมู่/ยี่ห้อ/รุ่น) และ sort ได้ทุกคอลัมน์ (ทีมขอเพิ่มระหว่างรอ seed production) | ไม่มี backend test แยก (ไม่มี logic ใหม่ฝั่ง server, filter/sort ทำฝั่ง client ล้วน) | ✅ Implemented — verify ผ่าน browser จริงทั้ง Admin (`admin/Dashboard.jsx`) และ Branch (`branch/Dashboard.jsx`) เห็นตัวเลข KPI + กราฟตรงกับข้อมูลจริงใน DB, responsive ตรวจแล้วบน mobile viewport, search/sort verify ด้วยการรัน filter จริงผ่าน JS (เช่น ค้นหา "GPU" ได้ 10/60 ตรงกับจำนวนจริง) |
